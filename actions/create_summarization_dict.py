...
def write_dictionary(dictionary=None):
    with open(os.path.join(os.path.dirname(__file__),'dict_for_summarization.json'), 'w') as fp:
        if dictionary is None:
            json.dump(create_dict_summarization(), fp)
        else:
...
def main():
    if not os.path.isfile('./dict_for_summarization.json'):
        write_dictionary()
    else:
